...
module.exports = generators.Base.extend({
  ...
  installAssets: function() {
    Promise.all(
      ["client/assets/favicon.png"].map(file => {
        return this.fs.copy(
  ...
  installLanguageTemplate: function() {
    if (this.config.get("clientLanguage") === "es6") {
      this.fs.copy(
        this.templatePath("client/internals"),
        this.destinationPath("client/internals"),
        this.answers
      );
      ...
      Promise.all(
        [
          ...
          "client/source/containers/home-view/style.css",
  ...
  installClientDependencies: function() {
    ...
    return this.fs.writeJSON(
      this.destinationPath(packageJsonPath),
      newPackageJson
    );
  },
